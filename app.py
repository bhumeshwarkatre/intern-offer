    ...
    if not all([intern_name, domain, email]):
        ...
        try:
            with open(docx_path, "rb") as f:
                upload_result = words_api.upload_file(UploadFileRequest(f, cloud_doc_name,storage_name=None))
            ...
            if not upload_result.uploaded or cloud_doc_name not in upload_result.uploaded:
                ...
                save_as_request = SaveAsRequest(name=cloud_doc_name, save_options_data=save_opts,storage_name=None)
                words_api.save_as(save_as_request)

            pdf_stream = words_api.download_file(DownloadFileRequest(cloud_pdf_name,storage_name=None))
